...
export class Refences1621203104065 implements MigrationInterface {
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    const attachmentTableForeignKey = attachmentTable.foreignKeys.find(
      (fk) => fk.columnNames.indexOf('todoId') !== -1,
    );
    await queryRunner.dropForeignKey('attachment', attachmentTableForeignKey);

    const userTable = await queryRunner.getTable('user');
    const userTableForeignKey = userTable.foreignKeys.find(
    ...
    const todoTableForeignKey = todoTable.foreignKeys.find(
